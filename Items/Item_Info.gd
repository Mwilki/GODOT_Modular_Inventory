...
var type_names = {
	...
	Game_Enums.EQUIPMENT_TYPE.OFF_HAND: "Offhand"
}
...
func display( slot : Inventory_Slot ):
	...
	var line_type = Item_Stat_Line.new(type_names[slot.item.equipment_type], "normal")
...
#
	...
	var height = 0
	for c in line_container.get_children():
		height += c.rect_size.y + 3 #3 = seperation
		if c.rect_size.x > max_width:
			max_width = c.rect_size.x
			
		rect_size = Vector2(max_width + 30, height + 8) #trial and error for padding #
